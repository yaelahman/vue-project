...
export default {
  ...
  methods: {
    ...
    menitTerlambat(val) {
      ...
      var now = val.t_absensi_Dates + ' ' + (val.work_personel !== null ? val.work_personel.get_work_schedule.m_work_schedule_clockIn : 0)
      // val.work_personel.get_work_schedule.m_work_schedule_clockIn
      var tolerance = val.work_personel !== null ? val.work_personel.get_work_pattern.m_work_patern_tolerance : 0;
    ...
    filterRangeDate() {
      if (this.filterType == 'show') {
        ...
        Swal.fire({
          ...
          if (resp.isConfirmed) {
            axios.get(`${env.VITE_API_URL}/exports/absensi?start_date=${this.search.startDate}&end_date=${this.search.endDate}`).then((resp) => {
              if (resp.status == 200) {
