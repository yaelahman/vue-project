    ...
    filterRangeDate() {
      if (this.filterType == "show") {
        ...
        Swal.fire({
          ...
          text:
            "Ingin Ekspor Kunjungan Dengan Tanggal " +
          ...
          if (resp.isConfirmed) {
            axios
              .get(
                `${env.VITE_API_URL}/exports/kunjungan?start_date=${this.search.startDate}&end_date=${this.search.endDate}`
              )
              .then((resp) => {
                if (resp.status == 200) {
