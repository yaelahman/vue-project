    ...
    filterRangeDate() {
      if (this.filterType == "show") {
        ...
        Swal.fire({
          ...
          text:
            "Ingin Ekspor Izin Cuti Dengan Tanggal " +
          ...
          if (resp.isConfirmed) {
            axios
              .get(
                `${env.VITE_API_URL}/exports/izin/3?start_date=${this.search.startDate}&end_date=${this.search.endDate}&status=${this.search.status}`
              )
              .then((resp) => {
                if (resp.status == 200) {
